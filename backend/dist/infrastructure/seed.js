...
async function generateHotelEmbedding(hotel) {
    const embeddingText = `${hotel.name} ${hotel.description} ${hotel.locationCity} ${hotel.locationCountry} ${hotel.amenities.join(' ')} ${hotel.rooms.map((r) => r.roomType).join(' ')}`;
    try {
        return await (0, embeddings_1.generateEmbedding)(embeddingText);
    }
    catch (error) {
...
async function seedDatabase() {
    try {
        ...
        await Promise.all([
            Location_1.default.deleteMany({}),
            Hotel_1.default.deleteMany({}),
            Amenity_1.Amenity.deleteMany({}),
            Review_1.default.deleteMany({})
        ]);
        // Seed amenities
        console.log('🏷️  Seeding amenities...');
        const amenities = await Amenity_1.Amenity.insertMany(AMENITIES_DATA);
        console.log(`✅ Created ${amenities.length} amenities`);
        // Seed locations
        console.log('📍 Seeding locations...');
        const locations = await Location_1.default.insertMany(LOCATIONS_DATA);
        ...
        const hotelPromises = HOTELS_DATA.map(async (hotelData) => {
            ...
            const priceStartingFrom = Math.min(...hotelData.rooms.map((r) => r.pricePerNight));
        ...
        const hotelUpdatePromises = hotels.map(async (hotel) => {
            if (!hotel)
                return null;
            const hotelReviews = reviews.filter(review => review.hotelId.toString() === hotel._id?.toString());
            ...
            const averageRating = hotelReviews.length > 0
                ...
                : 0;
            return Hotel_1.default.findByIdAndUpdate(hotel._id, {
                reviews: reviewIds,
        ...
        // Final statistics
        const stats = {
            locations: await Location_1.default.countDocuments(),
            hotels: await Hotel_1.default.countDocuments(),
            amenities: await Amenity_1.Amenity.countDocuments(),
            reviews: await Review_1.default.countDocuments()
        };
