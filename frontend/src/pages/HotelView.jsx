...
import { Star, MapPin, Users, Bed, Bath, CheckCircle, ArrowLeft, Heart, Share2 } from 'lucide-react';
import { Card, CardContent, CardHeader } from '@/src/components/ui/card';
import { Button } from '@/src/components/ui/button';
import { Badge } from '@/src/components/ui/badge';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/src/components/ui/tabs';
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/src/components/ui/carousel';
import { useGetHotelByIdQuery, useCreateBookingMutation } from '@/src/store/api';
...
export default function HotelView() {
  ...
  const selectedRoomData = hotel?.availableRooms?.find(r => r.roomNumber === selectedRoom);
  ...
  const totalPrice = selectedRoomData && numberOfNights > 0 ? 
    numberOfNights * selectedRoomData.pricePerNight : 0;
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const errorMessage = error?.data?.message || 'Failed to create booking. Please try again.';
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3">
                <div className="flex items-center">
                  ...
                  <span className="ml-1 font-semibold">{hotel.averageRating || 0}</span>
              ...
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-100">
                    <div className="text-xl sm:text-2xl font-bold text-blue-600">{hotel.totalRooms || 0}</div>
                    <div className="text-xs sm:text-sm text-blue-600/80 font-medium">Total Rooms</div>
                  </div>
                  <div className="text-center p-4 bg-green-50 rounded-lg border border-green-100">
                    <div className="text-xl sm:text-2xl font-bold text-green-600">{hotel.availableRoomsCount || 0}</div>
                    <div className="text-xs sm:text-sm text-green-600/80 font-medium">Available</div>
                  </div>
                  <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-100">
                    <div className="text-xl sm:text-2xl font-bold text-yellow-600">{hotel.averageRating || 0}</div>
                    <div className="text-xs sm:text-sm text-yellow-600/80 font-medium">Rating</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {hotel.amenities.map((amenity, index) => {
                      ...
                      const IconComponent = amenityIcons[0]?.icon || CheckCircle;
                      return (
              ...
              <Card className="border-0 shadow-sm border-l-4 border-l-green-500">
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-green-700">Available Rooms</h3>
                  </div>
                    ...
                    {hotel.availableRooms.map((room, index) => (
                      <div key={index} className="bg-green-50 border border-green-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <h4 className="font-semibold text-green-800">{room.roomType}</h4>
                            <p className="text-sm text-green-600">Room {room.roomNumber}</p>
                          </div>
                        ...
                        <div className="text-xl font-bold text-green-800">
                          Rs. {room.pricePerNight.toLocaleString()}<span className="text-sm font-normal">/night</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <Card className="border-0 shadow-sm border-l-4 border-l-red-500">
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-red-700">Currently Occupied</h3>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {hotel.unavailableRooms.map((room, index) => (
                      <div key={index} className="bg-red-50 border border-red-200 rounded-xl p-4 opacity-75">
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <h4 className="font-semibold text-red-800">{room.roomType}</h4>
                            <p className="text-sm text-red-600">Room {room.roomNumber}</p>
                          </div>
                          <Badge className="bg-red-100 text-red-800 border-red-200">Occupied</Badge>
                        </div>
          ...
          <div className="order-first lg:order-last">
            <Card className="sticky top-20 border-0 shadow-lg bg-gradient-to-br from-white to-gray-50/50">
              <CardContent className="p-6">
                <div className="text-center mb-6">
                  ...
                  <div className="text-3xl font-bold text-primary mb-1">
                    {selectedRoom ? (
                      (() => {
                        const room = hotel.availableRooms?.find(r => r.roomNumber === selectedRoom);
                        return `Rs. ${room?.pricePerNight.toLocaleString() || 0}`;
                      })()
                    ) : hotel.priceRange?.minPrice ? (
                      `Rs. ${hotel.priceRange.minPrice.toLocaleString()}`
                    ) : (
                    ...
                    >
                      ...
                      {hotel.availableRooms && hotel.availableRooms.map((room) => (
                        <option key={room.roomNumber} value={room.roomNumber}>
                          {room.roomType} - Room {room.roomNumber} (Rs. {room.pricePerNight.toLocaleString()}/night)
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                {/* Booking Summary */}
                {selectedRoomData && checkIn && checkOut && numberOfNights > 0 && (
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
                    <h4 className="font-semibold text-gray-900 mb-3">Booking Summary</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Room:</span>
                        <span className="font-medium">{selectedRoomData.roomType} #{selectedRoomData.roomNumber}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Dates:</span>
                        <span className="font-medium">{new Date(checkIn).toLocaleDateString()} - {new Date(checkOut).toLocaleDateString()}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">Rs. {selectedRoomData.pricePerNight.toLocaleString()}</span>
                      </div>
                ...
                {userLoaded && user ? (
                  ...
                  <Button 
                    onClick={() => toast.info('Please sign in to make a booking')}
